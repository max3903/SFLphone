...
class VMStorageFile extends VMStorage {
	...
	*/
	public function rename($folderName, $oldName, $newName) {
			...
			if( strcmp($fol->getName(), $folderName) == 0 ) {
				...
				if( $vm != NULL ) {
					if( ! rename($this->pathFolder ."/". $folderName ."/". $oldName .".txt", $this->pathFolder ."/". $folderName ."/". $newName .".txt") ) {
						echo "<error>". $oldname ." can't be renamed</error>";
					}
					$arr = $vm->getFormats();
					foreach( $arr as $file ) {
						$basename = substr($file, strrpos($file, ".")+1, strlen($file));
						...
						if( ! rename($old, $new) ) {
							echo "<error>". $old ." can't be renamed</error>";
						}
					}
		...
		echo "<error>Voicemail ". $oldName ." not found</error>";
		return FALSE;
	}
	...
	*/
	public function deleteFolder( $folder ) {
			...
			if( strcmp($fol->getName(), $folder) == 0 ) {
				...
				foreach( $fol->getListVM() as $vm ) {
					if( ! unlink($this->pathFolder ."/". $folder ."/". $vmName .".txt") ) {
						echo "<error>". $vm ." wasn't deleted</error>";
						$delete = FALSE;
					}
					$arr = $vm->getFormats();
					foreach( $arr as $file ) {
						...
						if( ! unlink($this->pathFolder ."/". $folder ."/". $vmName .".". $basename) ) {
							echo "<error>". $file ." wasn't deleted</error>";
							$delete = FALSE;
						}
					}
				}
		...
		echo "<error>File ". $vmName ." not found</error>";
		return FALSE;
	}
			...
			if( strcmp($fol->getName(), $folder) == 0 ) {
				...
				if( $vm != NULL ) {
					...
					if( @unlink($this->pathFolder ."/". $folder ."/". $vmName .".txt") ) {
						...
						echo "<error>". $vm->getName() ." wasn't deleted</error>";
						$delete = FALSE;
					}
					return $delete;
				}
