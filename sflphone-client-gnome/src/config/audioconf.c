 ...
 */
void
preferences_dialog_fill_codec_list (account_t **account)
{
  // Get model of view and clear it
  GtkListStore* codecStore =
      GTK_LIST_STORE (gtk_tree_view_get_model (GTK_TREE_VIEW (codecTreeView)));
  gtk_list_store_clear (codecStore);

  // Get all the codecs for this account
  codec_library_load_audio_codecs_by_account(*account);
  GQueue* current = codec_library_get_audio_codecs((*account)->codecs);
...
codec_move (gboolean moveUp, gpointer data)
{
  ...
  gtk_tree_view_scroll_to_cell (GTK_TREE_VIEW (codecTreeView), treePath, NULL,
      FALSE, 0, 0);
...
GtkWidget* create_audio_codec_box (account_t **a)
{
  GtkWidget *ret;
  ...
  codecStore = gtk_list_store_new (CODEC_COLUMN_COUNT, G_TYPE_BOOLEAN, // Active
      G_TYPE_STRING, // Name
  ...
  treeViewColumn = gtk_tree_view_column_new_with_attributes ("", renderer,
      "active", COLUMN_CODEC_ACTIVE, NULL);
  ...
  treeViewColumn = gtk_tree_view_column_new_with_attributes (_("Name"),
