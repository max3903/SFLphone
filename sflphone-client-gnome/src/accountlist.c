...
{
  GList * c =
      g_queue_find_custom (accountQueue, accountID, is_accountID_struct);
  if (c)
    {
  ...
  for (i = 0; i < account_list_get_size (); i++)
    {
      ...
      if(account && (account->state == ACCOUNT_STATE_REGISTERED)) {
        res++;
      }
    }
...
gchar*
account_list_get_current_id (void)
{
...
int
account_list_get_sip_account_number (void)
{
  ...
  for (i = 0; i < size; i++)
    {
      ...
      if (strcmp (g_hash_table_lookup (current->properties, ACCOUNT_TYPE),
          "SIP") == 0)
        n++;
    }
...
int
account_list_get_iax_account_number (void)
{
...
gchar *
account_list_get_ordered_list (void)
{
...
guint
account_list_get_position (account_t *account)
{
  guint size, i;
  account_t *tmp;
...
current_account_has_mailbox (void)
{
  ...
  if (current)
    {
      if (g_strcasecmp (g_hash_table_lookup (current->properties,
          ACCOUNT_MAILBOX), "") != 0)
        return TRUE;
    }
