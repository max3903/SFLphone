...
CodecFactory::CodecFactory() :
	_defaultCodecOrder(),  _Cache(), _nbCodecs(), _CodecInMemory(), _codecsMap() {
}
...
void CodecFactory::init() {
	...
	for (i = 0; i < _nbCodecs; i++) {
		_codecsMap[(AudioCodecType) CodecDynamicList[i]->getPayloadType()]
				= CodecDynamicList[i];
		_debug ("%s" , CodecDynamicList[i]->getMimeSubtype().c_str());
	}
}

std::vector<std::string> CodecFactory::getAvailableCodecMimeType() {
	std::vector<std::string> output;
	PayloadTypeToCodecMapIterator it;
...
CodecFactory::getCodec(AudioCodecType payload) {
	...
	_error ("CodecDescriptor: Error cannot found codec %i in _CodecsMap from codec descriptor", payload);

	return NULL;
}

AudioCodec* CodecFactory::getCodec(const std::string& mimeSubtype) {
...
std::vector<AudioCodec*> CodecFactory::scanCodecDirectory(void) {
...
AudioCodec* CodecFactory::instantiateCodec(AudioCodecType payload) {
...
AudioCodec* CodecFactory::getFirstCodecAvailable(void) {
	PayloadTypeToCodecMap::iterator iter = _codecsMap.begin();
...
bool CodecFactory::alreadyInCache(std::string lib) {
	int i;
