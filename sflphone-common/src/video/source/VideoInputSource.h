...
public:
	...
	/**
	 * @return The current video format for the output video frames.
	 * @precondition The source must be running.
	 * @see sfl#VideoInputSource#open
	 */
	...
	/**
	 * @param deviceFile The device file representing this device (eg /dev/video0).
	 * @param dim The supported native dimension to tell this device to capture into (does not require scaling).
	 * @param rate The supported native framerate to tell this device to capture at (does not require video rate adjustment).
	 * @throw UnknownVideoDeviceException if no device with the given device file can be found.
	 */
	void setDevice(const std::string& deviceFile, Dimension dim, FrameRate rate) throw (UnknownVideoDeviceException);

	/**
	 * @return the current device that is being used.
	 */
	inline VideoDevicePtr getDevice() const {
