...
#include "CodecFactory.h" // CodecFactory class contained by value here
 ...
 * A great example of the concept of a "god class" :)
 * Please make sure not to make things worse, and try
 * to remove as much as you can from here.
 *
 * Here is a deal : if you add, remove (but improve!) something before.
 */
class ManagerImpl {
public:
	/**
	 ...
	 */
	void
	incomingMessage(const AccountID& accountId, const std::string& message);
	 ...
	 *			false otherwise
	 */
	bool getConfig(const std::string& section, const std::string& name,
			TokenList& arg);
	 ...
	 */
	std::string getConfigString(const std::string& section,
	 ...
	 */
	void
			initConfigFile(bool load_user_value = true, std::string alternate =
					"");
...
public:
	...
	AccountID getAccountIdFromNameAndServer(const std::string& userName,
