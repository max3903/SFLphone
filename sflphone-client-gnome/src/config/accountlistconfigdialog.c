      ...
      if (account)
        {
          ...
          DEBUG("Filling accounts: Account is enabled :%s", g_hash_table_lookup(account->properties, ACCOUNT_ENABLED));

          gtk_list_store_set (accountStore, &iter,
              COLUMN_ACCOUNT_ALIAS,
              g_hash_table_lookup (account->properties, ACCOUNT_ALIAS), // Name
              COLUMN_ACCOUNT_TYPE,
              g_hash_table_lookup (account->properties, ACCOUNT_TYPE), // Protocol
              COLUMN_ACCOUNT_STATUS,
              account_state_name (account->state), // Status
              COLUMN_ACCOUNT_ACTIVE,
              (g_strcasecmp (g_hash_table_lookup (account->properties,
                  ACCOUNT_ENABLED), "true") == 0) ? TRUE : FALSE, // Enable/Disable
              COLUMN_ACCOUNT_DATA, account, // Pointer
              -1);
        }
    }
      ...
      if (g_strcasecmp (selectedAccount->accountID, IP2IP) != 0)
        {
          ...
          if (selectedAccount->protocol_state_description != NULL
              && selectedAccount->protocol_state_code != 0)
            {
              ...
              gchar * message = g_strconcat (account_state_name (
                  selectedAccount->state), ". ", response, NULL);
...
static void
help_contents_cb (GtkWidget * widget, gpointer data UNUSED)
{
  GError *error = NULL;
...
static void
close_dialog_cb (GtkWidget * widget, gpointer data UNUSED)
{
...
void
highlight_ip_profile (GtkTreeViewColumn *col, GtkCellRenderer *rend,
  ...
  if (current != NULL)
    {
      ...
      (g_strcasecmp (current->accountID, IP2IP) == 0) ? g_object_set (
          G_OBJECT (rend), "weight", PANGO_WEIGHT_THIN, "style",
          PANGO_STYLE_ITALIC, "stretch", PANGO_STRETCH_ULTRA_EXPANDED, "scale",
          0.95, NULL) : g_object_set (G_OBJECT (rend), "weight",
          PANGO_WEIGHT_MEDIUM, "style", PANGO_STYLE_NORMAL, "stretch",
          PANGO_STRETCH_NORMAL, "scale", 1.0, NULL);
    }
}

void
highlight_registration (GtkTreeViewColumn *col, GtkCellRenderer *rend,
    GtkTreeModel *tree_model, GtkTreeIter *iter, gpointer data)
{
      ...
      if (g_strcasecmp (current->accountID, IP2IP) != 0)
        {
          ...
          (current->state == ACCOUNT_STATE_REGISTERED) ? g_object_set (
              G_OBJECT (rend), "foreground", "Dark Green", NULL)
...
create_account_list (GtkDialog * dialog)
{
  ...
  gtk_scrolled_window_set_shadow_type (GTK_SCROLLED_WINDOW(scrolledWindow),
      GTK_SHADOW_IN);
  gtk_table_attach (GTK_TABLE(table), scrolledWindow, 0, 1, 0, 1, GTK_EXPAND
      | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);

  accountStore = gtk_list_store_new (COLUMN_ACCOUNT_COUNT, G_TYPE_STRING, // Name
  ...
  treeView
      = GTK_TREE_VIEW (gtk_tree_view_new_with_model (GTK_TREE_MODEL(accountStore)));
  ...
  treeViewColumn = gtk_tree_view_column_new_with_attributes ("Enabled",
      renderer, "active", COLUMN_ACCOUNT_ACTIVE, NULL);
  ...
  treeViewColumn = gtk_tree_view_column_new_with_attributes ("Alias", renderer,
      "markup", COLUMN_ACCOUNT_ALIAS, NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW(treeView), treeViewColumn);
  ...
  gtk_tree_view_column_set_cell_data_func (treeViewColumn, renderer,
      highlight_ip_profile, NULL, NULL);

  renderer = gtk_cell_renderer_text_new ();
  treeViewColumn = gtk_tree_view_column_new_with_attributes (_("Protocol"),
      renderer, "markup", COLUMN_ACCOUNT_TYPE, NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW(treeView), treeViewColumn);
  gtk_tree_view_column_set_cell_data_func (treeViewColumn, renderer,
      highlight_ip_profile, NULL, NULL);
  ...
  treeViewColumn = gtk_tree_view_column_new_with_attributes (_("Status"),
      renderer, "markup", COLUMN_ACCOUNT_STATUS, NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW(treeView), treeViewColumn);
  ...
  gtk_tree_view_column_set_cell_data_func (treeViewColumn, renderer,
      highlight_registration, NULL, NULL);
  ...
  gtk_table_attach (GTK_TABLE(table), buttonBox, 1, 2, 0, 1, GTK_EXPAND
      | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_widget_set_sensitive (GTK_WIDGET(accountMoveDownButton), FALSE);
  gtk_box_pack_start (GTK_BOX(buttonBox), accountMoveDownButton, FALSE, FALSE,
      0);
  ...
  gtk_table_attach (GTK_TABLE(table), buttonHbox, 0, 2, 1, 2, GTK_EXPAND
  ...
  gtk_widget_set_size_request (GTK_WIDGET(scrolledWindow), requisition.width
      + 20, requisition.height);
  GtkRequisition requisitionButton;
  gtk_widget_size_request (GTK_WIDGET(deleteButton), &requisitionButton);
  gtk_widget_set_size_request (GTK_WIDGET(closeButton),
      requisitionButton.width, -1);
  gtk_widget_set_size_request (GTK_WIDGET(helpButton), requisitionButton.width,
      -1);
...
show_account_list_config_dialog (void)
{
  ...
  gtk_box_pack_start (GTK_BOX(accountListDialog->vbox ), accountFrame, TRUE,
      TRUE, 0);
  ...
  gtk_box_pack_start (GTK_BOX(accountListDialog->vbox ), status_bar, TRUE,
      TRUE, 0);
  ...
  if (number_accounts)
    {
      gchar * message = g_strdup_printf (n_("There is %d active account",
          "There are %d active accounts", number_accounts), number_accounts);
  ...
  DEBUG("About to destroy account list dialog ...");
