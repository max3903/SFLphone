 ...
 */
static void
show_password_cb (GtkWidget *widget, gpointer data)
{
  gtk_entry_set_visibility (GTK_ENTRY (data), !gtk_entry_get_visibility (
      GTK_ENTRY (data)));
}
...
change_protocol_cb (account_t *currentAccount UNUSED)
{
  ...
  gchar *protocol = gtk_combo_box_get_active_text (
      GTK_COMBO_BOX (protocolComboBox));
...
static GPtrArray*
getNewCredential (GHashTable * properties)
{
  ...
  valid
      = gtk_tree_model_get_iter_first (GTK_TREE_MODEL(credentialStore), &iter);
  ...
  gtk_tree_model_get (GTK_TREE_MODEL(credentialStore), &iter,
      COLUMN_CREDENTIAL_REALM, &realm, COLUMN_CREDENTIAL_USERNAME, &username,
      COLUMN_CREDENTIAL_PASSWORD, &password, -1);
  ...
  while (valid)
    {
      gtk_tree_model_get (GTK_TREE_MODEL(credentialStore), &iter,
          COLUMN_CREDENTIAL_REALM, &realm, COLUMN_CREDENTIAL_USERNAME,
          &username, COLUMN_CREDENTIAL_PASSWORD, &password, -1);
...
static void
update_credential_cb (GtkWidget *widget, gpointer data UNUSED)
{
  GtkTreeIter iter;
  gtk_tree_model_get_iter_from_string ((GtkTreeModel *) credentialStore, &iter,
      "0");
  ...
  gtk_list_store_set (GTK_LIST_STORE (credentialStore), &iter, column,
      (gchar *) gtk_entry_get_text (GTK_ENTRY(widget)), -1);
}

static GtkWidget*
create_basic_tab (account_t **a)
{
  ...
  if (currentAccount)
    {
      ...
      curAccountType = g_hash_table_lookup (currentAccount->properties,
          ACCOUNT_TYPE);
      curAccountEnabled = g_hash_table_lookup (currentAccount->properties,
          ACCOUNT_ENABLED);
      curAlias
          = g_hash_table_lookup (currentAccount->properties, ACCOUNT_ALIAS);
      curHostname = g_hash_table_lookup (currentAccount->properties,
          ACCOUNT_HOSTNAME);
      curPassword = g_hash_table_lookup (currentAccount->properties,
          ACCOUNT_PASSWORD);
      curUsername = g_hash_table_lookup (currentAccount->properties,
          ACCOUNT_USERNAME);
      // curRouteSet = g_hash_table_lookup(currentAccount->properties, ACCOUNT_ROUTE);
      curMailbox = g_hash_table_lookup (currentAccount->properties,
          ACCOUNT_MAILBOX);
      curUseragent = g_hash_table_lookup (currentAccount->properties,
          ACCOUNT_USERAGENT);
    }
  ...
  gtk_table_attach (GTK_TABLE( table ), label, 0, 1, row, row + 1, GTK_EXPAND
  ...
  gtk_table_attach (GTK_TABLE( table ), entryAlias, 1, 2, row, row + 1,
  ...
  gtk_table_attach (GTK_TABLE( table ), protocolComboBox, 1, 2, row, row + 1,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_table_attach (GTK_TABLE( table ), entryHostname, 1, 2, row, row + 1,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_entry_set_icon_from_pixbuf (GTK_ENTRY (entryUsername),
      GTK_ENTRY_ICON_PRIMARY, gdk_pixbuf_new_from_file (
          ICONS_DIR "/stock_person.svg", NULL));
#else
  ...
  gtk_table_attach (GTK_TABLE( table ), entryUsername, 1, 2, row, row + 1,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  if (strcmp (curAccountType, "SIP") == 0)
    {
      g_signal_connect(G_OBJECT (entryUsername), "changed", G_CALLBACK (update_credential_cb), NULL);
      g_object_set_data (G_OBJECT (entryUsername), "column", GINT_TO_POINTER (
          COLUMN_CREDENTIAL_USERNAME));
    }
  ...
  gtk_entry_set_icon_from_stock (GTK_ENTRY (entryPassword),
  ...
  gtk_table_attach (GTK_TABLE( table ), entryPassword, 1, 2, row, row + 1,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  if (strcmp (curAccountType, "SIP") == 0)
    {
      g_signal_connect (G_OBJECT (entryPassword), "changed", G_CALLBACK (update_credential_cb), NULL);
      g_object_set_data (G_OBJECT (entryPassword), "column", GINT_TO_POINTER (
          COLUMN_CREDENTIAL_PASSWORD));
    }
  ...
  gtk_table_attach (GTK_TABLE (table), clearTextCheckbox, 1, 2, row, row + 1,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_table_attach (GTK_TABLE( table ), entryMailbox, 1, 2, row, row + 1,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_table_attach (GTK_TABLE( table ), entryUseragent, 1, 2, row, row + 1,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
...
static void
fill_treeview_with_credential (GtkListStore * credentialStore,
    account_t * account)
{
  GtkTreeIter iter;
  ...
  for (i = 0; i < account->credential_information->len; i++)
    {
      GHashTable * element =
          g_ptr_array_index(account->credential_information, i);
      gtk_list_store_append (credentialStore, &iter);
      gtk_list_store_set (credentialStore, &iter, COLUMN_CREDENTIAL_REALM,
          g_hash_table_lookup (element, ACCOUNT_REALM),
          COLUMN_CREDENTIAL_USERNAME, g_hash_table_lookup (element,
              ACCOUNT_USERNAME), COLUMN_CREDENTIAL_PASSWORD,
          g_hash_table_lookup (element, ACCOUNT_PASSWORD),
          COLUMN_CREDENTIAL_DATA, element, -1);
    }
}

static
select_credential_cb (GtkTreeSelection *selection, GtkTreeModel *model)
{
  GtkTreeIter iter;
  GtkTreePath *path;
...
static void
add_credential_cb (GtkWidget *button, gpointer data)
{
  GtkTreeIter iter;
  GtkTreeModel *model = (GtkTreeModel *) data;

  gtk_list_store_append (GTK_LIST_STORE (model), &iter);
  gtk_list_store_set (GTK_LIST_STORE (model), &iter, COLUMN_CREDENTIAL_REALM,
      "*", COLUMN_CREDENTIAL_USERNAME, _("Authentication"),
      COLUMN_CREDENTIAL_PASSWORD, _("Secret"), -1);
}

static void
delete_credential_cb (GtkWidget *button, gpointer data)
{
  GtkTreeIter iter;
    ...
    gpointer data)
{
  ...
  if (g_strcasecmp (path_desc, "0") == 0)
    {
      if (g_strcasecmp (text, gtk_entry_get_text (GTK_ENTRY(entryUsername)))
          != 0)
        {
          g_signal_handlers_disconnect_by_func (G_OBJECT(entryUsername), G_CALLBACK(update_credential_cb), NULL);
        }
    ...
    const gchar * path, gpointer data)
{
  ...
  if (g_strcasecmp (path, "0") == 0)
    {
      gtk_entry_set_text (GTK_ENTRY (editable), gtk_entry_get_text (
          GTK_ENTRY (entryPassword)));
    }
}

static void
show_advanced_zrtp_options_cb (GtkWidget *widget UNUSED, gpointer data)
{

  DEBUG("Advanced options for SRTP");
  if (g_strcasecmp (gtk_combo_box_get_active_text (
      GTK_COMBO_BOX(keyExchangeCombo)), (gchar *) "ZRTP") == 0)
    {
...
static void
key_exchange_changed_cb (GtkWidget *widget, gpointer data)
{

  DEBUG("Key exchange changed %s", gtk_combo_box_get_active_text(GTK_COMBO_BOX(keyExchangeCombo)));

  int isSdes = g_strcasecmp (gtk_combo_box_get_active_text (
      GTK_COMBO_BOX(keyExchangeCombo)), (gchar *) "SDES");
  int isZrtp = g_strcasecmp (gtk_combo_box_get_active_text (
      GTK_COMBO_BOX(keyExchangeCombo)), (gchar *) "ZRTP");
...
static void
use_sip_tls_cb (GtkWidget *widget, gpointer data)
{
  ...
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(widget)))
    {
      ...
      if (!gtk_toggle_button_get_active (
          GTK_TOGGLE_BUTTON(sameAsLocalRadioButton)))
        {
...
static
local_interface_changed_cb (GtkWidget * widget, gpointer data UNUSED)
{
  ...
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(sameAsLocalRadioButton)))
    {
      ...
      local_iface_name = (gchar *) gtk_combo_box_get_active_text (
          GTK_COMBO_BOX (localAddressCombo));
      // sflphone_get_interface_addr_from_name((char *)local_interface);
      sflphone_get_interface_addr_from_name (local_iface_name,
          &local_iface_addr, 36);
...
static
set_published_addr_manually_cb (GtkWidget * widget, gpointer data UNUSED)
{
...
static
use_stun_cb (GtkWidget *widget, gpointer data UNUSED)
{
  ...
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
  ...
  else
    {
      ...
      if (gtk_toggle_button_get_active (
          GTK_TOGGLE_BUTTON (publishedAddrRadioButton)))
        {
          gtk_widget_show (publishedAddressLabel);
          gtk_widget_show (publishedPortLabel);
          gtk_widget_show (publishedAddressEntry);
...
static
same_as_local_cb (GtkWidget * widget, gpointer data UNUSED)
{
  ...
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(widget)))
    {
      ...
      local_interface = (gchar *) gtk_combo_box_get_active_text (
          GTK_COMBO_BOX(localAddressCombo));
      // sflphone_get_interface_addr_from_name((char *)local_interface);
      ...
      gchar * local_port = (gchar *) gtk_entry_get_text (
          GTK_ENTRY(localPortSpinBox));
      gtk_spin_button_set_value (GTK_SPIN_BUTTON(publishedPortSpinBox),
          g_ascii_strtod (local_port, NULL));
    }
...
create_credential_widget (account_t **a)
{
  ...
  gtk_scrolled_window_set_policy (
      GTK_SCROLLED_WINDOW (scrolledWindowCredential), GTK_POLICY_AUTOMATIC,
      GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_shadow_type (
      GTK_SCROLLED_WINDOW (scrolledWindowCredential), GTK_SHADOW_IN);
  gtk_table_attach_defaults (GTK_TABLE (table), scrolledWindowCredential, 0, 1,
      0, 1);

  credentialStore = gtk_list_store_new (COLUMN_CREDENTIAL_COUNT, G_TYPE_STRING, // Realm
  ...
  treeViewCredential = gtk_tree_view_new_with_model (
      GTK_TREE_MODEL(credentialStore));
  treeSelection = gtk_tree_view_get_selection (
      GTK_TREE_VIEW (treeViewCredential));
  ...
  g_object_set_data (G_OBJECT (renderer), "column", GINT_TO_POINTER (
      COLUMN_CREDENTIAL_REALM));
  treeViewColumn = gtk_tree_view_column_new_with_attributes ("Realm", renderer,
      "markup", COLUMN_CREDENTIAL_REALM, NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW(treeViewCredential),
      treeViewColumn);
  ...
  treeViewColumn = gtk_tree_view_column_new_with_attributes (
      _("Authentication name"), renderer, "markup",
      COLUMN_CREDENTIAL_USERNAME, NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW(treeViewCredential),
      treeViewColumn);
  ...
  gtk_container_add (GTK_CONTAINER(scrolledWindowCredential),
      treeViewCredential);
...
create_security_widget (account_t **a)
{
  ...
  if ((*a))
    {
      curKeyExchange = g_hash_table_lookup ((*a)->properties,
      ...
      curSRTPEnabled = g_hash_table_lookup ((*a)->properties,
  ...
  gtk_table_attach_defaults (GTK_TABLE (table), sipTlsAdvancedButton, 2, 3, 0,
      1);
  ...
  useSipTlsCheckBox = gtk_check_button_new_with_mnemonic (
      _("Use TLS transport (sips)"));
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(useSipTlsCheckBox),
      (g_strcmp0 (curTLSEnabled, "true") == 0) ? TRUE : FALSE);
...
GtkWidget*
create_registration_expire (account_t **a)
{
  ...
  if (*a)
    {
      resolve_once = g_hash_table_lookup ((*a)->properties,
          ACCOUNT_RESOLVE_ONCE);
      account_expire = g_hash_table_lookup ((*a)->properties,
  ...
  gtk_spin_button_set_value (GTK_SPIN_BUTTON (expireSpinBox), g_ascii_strtod (
      account_expire, NULL));
  gtk_table_attach_defaults (GTK_TABLE (table), expireSpinBox, 1, 2, 0, 1);

  entryResolveNameOnlyOnce = gtk_check_button_new_with_mnemonic (
      _("_Comply with RFC 3263"));
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (entryResolveNameOnlyOnce),
      g_strcasecmp (resolve_once, "false") == 0 ? TRUE : FALSE);
  gtk_table_attach_defaults (GTK_TABLE (table), entryResolveNameOnlyOnce, 0, 2,
      1, 2);
...
create_network (account_t **a)
{
      ...
      if (!iface_found)
        {
          ...
          gtk_tree_model_get_iter_first (GTK_TREE_MODEL(ipInterfaceListStore),
              &current_local_iface_iter);
        }

    }

  localAddressCombo = gtk_combo_box_new_with_model (
      GTK_TREE_MODEL(ipInterfaceListStore));
  gtk_label_set_mnemonic_widget (GTK_LABEL (label), localAddressCombo);
  gtk_table_attach (GTK_TABLE( table ), localAddressCombo, 1, 2, 0, 1,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_cell_layout_pack_start (GTK_CELL_LAYOUT(localAddressCombo),
      ipInterfaceCellRenderer, TRUE);
  gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT(localAddressCombo),
      ipInterfaceCellRenderer, "text", 0, NULL);
  gtk_combo_box_set_active_iter (GTK_COMBO_BOX(localAddressCombo),
      &current_local_iface_iter);
  ...
  gtk_table_attach (GTK_TABLE( table ), localAddressEntry, 2, 3, 0, 1,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_spin_button_set_value (GTK_SPIN_BUTTON(localPortSpinBox), g_ascii_strtod (
      local_port, NULL));
...
GtkWidget*
create_published_address (account_t **a)
{

  GtkWidget *table, *frame, *label;
  gchar *use_tls, *published_address, *published_port, *local_address,
      *stun_enable, *stun_server, *published_sameas_local;
  ...
  if (*a)
    {
      ...
      published_sameas_local = g_hash_table_lookup ((*a)->properties,
          PUBLISHED_SAMEAS_LOCAL);

      if (g_strcasecmp (published_sameas_local, "true") == 0)
        {
          published_address = dbus_get_address_from_interface_name (
              g_hash_table_lookup ((*a)->properties, LOCAL_INTERFACE));
          published_port = g_hash_table_lookup ((*a)->properties, LOCAL_PORT);
        }
      else
        {
          published_address = g_hash_table_lookup ((*a)->properties,
              PUBLISHED_ADDRESS);
          published_port = g_hash_table_lookup ((*a)->properties,
              PUBLISHED_PORT);
        }

      stun_enable = g_hash_table_lookup ((*a)->properties,
          ACCOUNT_SIP_STUN_ENABLED);
      stun_server = g_hash_table_lookup ((*a)->properties,
  ...
  gtk_widget_set_sensitive (GTK_WIDGET(useStunCheckBox), g_strcasecmp (use_tls,
      "true") == 0 ? FALSE : TRUE);
  ...
  sameAsLocalRadioButton = gtk_radio_button_new_with_mnemonic_from_widget (
      NULL, _("Same as local parameters"));
  gtk_table_attach_defaults (GTK_TABLE(table), sameAsLocalRadioButton, 0, 2, 3,
      4);

  publishedAddrRadioButton = gtk_radio_button_new_with_mnemonic_from_widget (
      GTK_RADIO_BUTTON(sameAsLocalRadioButton),
      _("Set published address and port:"));
  gtk_table_attach_defaults (GTK_TABLE(table), publishedAddrRadioButton, 0, 2,
      4, 5);

  if (g_strcasecmp (published_sameas_local, "true") == 0)
    {
      gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (sameAsLocalRadioButton),
          TRUE);
      gtk_toggle_button_set_active (
          GTK_TOGGLE_BUTTON (publishedAddrRadioButton), FALSE);
    }
  else
    {
      gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (sameAsLocalRadioButton),
  ...
  gtk_label_set_mnemonic_widget (GTK_LABEL (publishedPortLabel),
      publishedPortSpinBox);
  gtk_spin_button_set_value (GTK_SPIN_BUTTON(publishedPortSpinBox),
      g_ascii_strtod (published_port, NULL));
  ...
  if (g_strcmp0 (p, "SIP") == 0)
    {
      ...
      gtk_table_attach (GTK_TABLE( table ), sipinfo, 1, 2, 0, 1, GTK_EXPAND
          | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
    }
...
show_account_window (account_t * account)
{
  ...
  gtk_notebook_append_page (GTK_NOTEBOOK (notebook), codecs_tab, gtk_label_new (
      _("Audio")));
  ...
  if (protocolComboBox)
    currentProtocol = (gchar *) gtk_combo_box_get_active_text (
        GTK_COMBO_BOX(protocolComboBox));
  ...
  if (g_strcasecmp (currentAccount->accountID, IP2IP) != 0)
    {
      ...
      gtk_notebook_append_page (GTK_NOTEBOOK(notebook), security_tab,
  ...
  else
    {
      ...
      gtk_notebook_prepend_page (GTK_NOTEBOOK (notebook), ip_tab,
  ...
  if (g_strcasecmp (currentAccount->accountID, IP2IP) != 0)
    {
      ...
      g_hash_table_replace (currentAccount->properties,
          g_strdup (ACCOUNT_ALIAS), g_strdup ((gchar *) gtk_entry_get_text (
              GTK_ENTRY(entryAlias))));
      g_hash_table_replace (currentAccount->properties,
          g_strdup (ACCOUNT_TYPE), g_strdup (proto));
      g_hash_table_replace (currentAccount->properties, g_strdup (
          ACCOUNT_HOSTNAME), g_strdup ((gchar *) gtk_entry_get_text (
          GTK_ENTRY(entryHostname))));
      g_hash_table_replace (currentAccount->properties, g_strdup (
          ACCOUNT_USERNAME), g_strdup ((gchar *) gtk_entry_get_text (
          GTK_ENTRY(entryUsername))));
      g_hash_table_replace (currentAccount->properties, g_strdup (
          ACCOUNT_PASSWORD), g_strdup ((gchar *) gtk_entry_get_text (
          GTK_ENTRY(entryPassword))));
      g_hash_table_replace (currentAccount->properties, g_strdup (
          ACCOUNT_MAILBOX), g_strdup ((gchar *) gtk_entry_get_text (
          GTK_ENTRY(entryMailbox))));

      // Variable used to update credentials
      current_username = (gchar *) g_hash_table_lookup (
          currentAccount->properties, g_strdup (ACCOUNT_USERNAME));
      ...
      if (g_strcasecmp (currentAccount->accountID, IP2IP) != 0)
        {
          ...
          g_hash_table_replace (currentAccount->properties, g_strdup (
              ACCOUNT_RESOLVE_ONCE), g_strdup (gtk_toggle_button_get_active (
              GTK_TOGGLE_BUTTON(entryResolveNameOnlyOnce)) ? "false" : "true"));

          g_hash_table_replace (currentAccount->properties, g_strdup (
              ACCOUNT_REGISTRATION_EXPIRE), g_strdup (
          ...
          g_hash_table_replace (currentAccount->properties, g_strdup (
              ACCOUNT_USERAGENT), g_strdup (gtk_entry_get_text (
              GTK_ENTRY(entryUseragent))));

          g_hash_table_replace (currentAccount->properties, g_strdup (
              ACCOUNT_SIP_STUN_ENABLED),
              g_strdup (gtk_toggle_button_get_active (
                  GTK_TOGGLE_BUTTON(useStunCheckBox)) ? "true" : "false"));

          g_hash_table_replace (currentAccount->properties, g_strdup (
              ACCOUNT_SIP_STUN_SERVER), g_strdup (gtk_entry_get_text (
              GTK_ENTRY(stunServerEntry))));

          g_hash_table_replace (currentAccount->properties, g_strdup (
              PUBLISHED_SAMEAS_LOCAL), g_strdup (gtk_toggle_button_get_active (
              GTK_TOGGLE_BUTTON(sameAsLocalRadioButton)) ? "true" : "false"));
          ...
          if (!gtk_toggle_button_get_active (
              GTK_TOGGLE_BUTTON(sameAsLocalRadioButton)))
            {
              ...
              g_hash_table_replace (currentAccount->properties, g_strdup (
                  PUBLISHED_PORT), g_strdup ((gchar *) gtk_entry_get_text (
                  GTK_ENTRY(publishedPortSpinBox))));

              g_hash_table_replace (currentAccount->properties, g_strdup (
                  PUBLISHED_ADDRESS), g_strdup ((gchar *) gtk_entry_get_text (
          ...
          if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(overrtp)))
            {
              g_hash_table_replace (currentAccount->properties, g_strdup (
                  ACCOUNT_DTMF_TYPE), g_strdup (OVERRTP));
            }
          else
            {
              g_hash_table_replace (currentAccount->properties, g_strdup (
                  ACCOUNT_DTMF_TYPE), g_strdup (SIPINFO));
            }

          gchar* keyExchange = (gchar *) gtk_combo_box_get_active_text (
              GTK_COMBO_BOX(keyExchangeCombo));

          if (g_strcasecmp (keyExchange, "ZRTP") == 0)
            {
              g_hash_table_replace (currentAccount->properties, g_strdup (
                  ACCOUNT_SRTP_ENABLED), g_strdup ("true"));
              g_hash_table_replace (currentAccount->properties, g_strdup (
                  ACCOUNT_KEY_EXCHANGE), g_strdup (ZRTP));
            }
          ...
          else if (g_strcasecmp (keyExchange, "SDES") == 0)
            {
              g_hash_table_replace (currentAccount->properties, g_strdup (
                  ACCOUNT_SRTP_ENABLED), g_strdup ("true"));
              g_hash_table_replace (currentAccount->properties, g_strdup (
                  ACCOUNT_KEY_EXCHANGE), g_strdup (SDES));
            }

          else
            {
              g_hash_table_replace (currentAccount->properties, g_strdup (
                  ACCOUNT_SRTP_ENABLED), g_strdup ("false"));
            }

          g_hash_table_replace (currentAccount->properties, g_strdup (
              TLS_ENABLE), g_strdup (gtk_toggle_button_get_active (
              GTK_TOGGLE_BUTTON(useSipTlsCheckBox)) ? "true" : "false"));
        }

      gboolean toneEnabled = gtk_toggle_button_get_active (
          GTK_TOGGLE_BUTTON(enableTone));
      g_hash_table_replace (currentAccount->properties, g_strdup (
          CONFIG_RINGTONE_ENABLED), g_strdup (toneEnabled ? "true" : "false"));

      g_hash_table_replace (currentAccount->properties, g_strdup (
          CONFIG_RINGTONE_PATH), g_strdup (
          (gchar *) gtk_file_chooser_get_filename (
              GTK_FILE_CHOOSER( fileChooser ))));

      g_hash_table_replace (currentAccount->properties, g_strdup (
          LOCAL_INTERFACE), g_strdup ((gchar *) gtk_combo_box_get_active_text (
          GTK_COMBO_BOX(localAddressCombo))));

      g_hash_table_replace (currentAccount->properties, g_strdup (LOCAL_PORT),
      ...
      if (g_strcasecmp (currentAccount->accountID, IP2IP) != 0)
        {
          ...
          GPtrArray * credential =
              getNewCredential (currentAccount->properties);
