...
void
show_advanced_tls_options (GHashTable * properties)
{
  GtkDialog * tlsDialog;
  GtkWidget * ret;

  tlsDialog
      = GTK_DIALOG(gtk_dialog_new_with_buttons (_("Advanced options for TLS"),
  ...
  gchar
      * description =
          g_markup_printf_escaped (
              _("TLS transport can be used along with UDP for those calls that would\n"
  ...
  if (properties != NULL)
    {
      ...
      tls_certificate_file = g_hash_table_lookup (properties,
          TLS_CERTIFICATE_FILE);
      tls_private_key_file = g_hash_table_lookup (properties,
      ...
      require_client_certificate = g_hash_table_lookup (properties,
          TLS_REQUIRE_CLIENT_CERTIFICATE);
      negotiation_timeout_sec = g_hash_table_lookup (properties,
          TLS_NEGOTIATION_TIMEOUT_SEC);
      negotiation_timeout_msec = g_hash_table_lookup (properties,
  ...
  gtk_spin_button_set_value (GTK_SPIN_BUTTON(tlsListenerPort), g_ascii_strtod (
      tls_listener_port, NULL));
  ...
  caListFileChooser = gtk_file_chooser_button_new (
      _("Choose a CA list file (optional)"), GTK_FILE_CHOOSER_ACTION_OPEN);
  gtk_table_attach (GTK_TABLE(table), caListFileChooser, 1, 2, 3, 4, GTK_EXPAND
  ...
  else
    {
      if (g_strcmp0 (tls_ca_list_file, "") == 0)
      ...
      else
        {
          ...
          gtk_file_chooser_set_file (GTK_FILE_CHOOSER(caListFileChooser), file,
              NULL);
          g_object_unref (file);
        }
    }
  ...
  certificateFileChooser = gtk_file_chooser_button_new (
      _("Choose a public endpoint certificate (optional)"),
      GTK_FILE_CHOOSER_ACTION_OPEN);
  gtk_table_attach (GTK_TABLE(table), certificateFileChooser, 1, 2, 4, 5,
      ...
      if (g_strcmp0 (tls_certificate_file, "") == 0)
        {
          ...
          gtk_file_chooser_unselect_all (
              GTK_FILE_CHOOSER(certificateFileChooser));
        }
      else
        {

          GFile * file = g_file_new_for_path (tls_certificate_file);
          gtk_file_chooser_set_file (GTK_FILE_CHOOSER(certificateFileChooser),
              file, NULL);
          g_object_unref (file);
        }
    }
  ...
  gtk_misc_set_alignment (GTK_MISC(label), 0, 0.5);
  gtk_table_attach (GTK_TABLE(table), label, 0, 1, 5, 6, GTK_EXPAND | GTK_FILL,
      GTK_EXPAND | GTK_FILL, 0, 0);
  GtkWidget * privateKeyFileChooser;
  privateKeyFileChooser
      = gtk_file_chooser_button_new (_("Choose a private key file (optional)"),
          GTK_FILE_CHOOSER_ACTION_OPEN);
  gtk_table_attach (GTK_TABLE(table), privateKeyFileChooser, 1, 2, 5, 6,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  else
    {
      ...
      if (g_strcmp0 (tls_private_key_file, "") == 0)
      ...
      else
        {
          ...
          gtk_file_chooser_set_file (GTK_FILE_CHOOSER(privateKeyFileChooser),
              file, NULL);
          g_object_unref (file);
  ...
  gtk_misc_set_alignment (GTK_MISC(label), 0, 0.5);
  gtk_table_attach (GTK_TABLE(table), label, 0, 1, 6, 7, GTK_EXPAND | GTK_FILL,
      GTK_EXPAND | GTK_FILL, 0, 0);
  GtkWidget * privateKeyPasswordEntry;
#if GTK_CHECK_VERSION(2,16,0)
  privateKeyPasswordEntry = gtk_entry_new ();
  gtk_entry_set_icon_from_stock (GTK_ENTRY (privateKeyPasswordEntry),
  ...
  gtk_table_attach (GTK_TABLE(table), privateKeyPasswordEntry, 1, 2, 6, 7,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_table_attach (GTK_TABLE(table), label, 0, 1, 7, 8, GTK_EXPAND | GTK_FILL,
      GTK_EXPAND | GTK_FILL, 0, 0);
  gtk_misc_set_alignment (GTK_MISC (label), 0, 0.5);
      ...
      for (supported_tls_method_ptr = supported_tls_method; *supported_tls_method_ptr; supported_tls_method_ptr++)
        {
          ...
          gtk_list_store_set (tlsProtocolMethodListStore, &iter, 0,
              *supported_tls_method_ptr, -1);
  ...
  tlsProtocolMethodCombo = gtk_combo_box_new_with_model (
      GTK_TREE_MODEL(tlsProtocolMethodListStore));
  gtk_label_set_mnemonic_widget (GTK_LABEL(label), tlsProtocolMethodCombo);
  gtk_table_attach (GTK_TABLE(table), tlsProtocolMethodCombo, 1, 2, 7, 8,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_cell_layout_pack_start (GTK_CELL_LAYOUT(tlsProtocolMethodCombo),
      tlsProtocolMethodCellRenderer, TRUE);
  gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT(tlsProtocolMethodCombo),
      tlsProtocolMethodCellRenderer, "text", 0, NULL);
  gtk_combo_box_set_active_iter (GTK_COMBO_BOX(tlsProtocolMethodCombo),
      &supported_tls_method_iter);

  /* Cipher list */
  GtkWidget * cipherListEntry;
  label = gtk_label_new_with_mnemonic (_("TLS cipher list"));
  gtk_table_attach (GTK_TABLE(table), label, 0, 1, 8, 9, GTK_EXPAND | GTK_FILL,
      GTK_EXPAND | GTK_FILL, 0, 0);
  gtk_misc_set_alignment (GTK_MISC(label), 0, 0.5);
  ...
  gtk_table_attach (GTK_TABLE(table), cipherListEntry, 1, 2, 8, 9, GTK_EXPAND
      | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);

  GtkWidget * serverNameInstance;
  label = gtk_label_new_with_mnemonic (
      _("Server name instance for outgoing TLS connection"));
  ...
  gtk_table_attach (GTK_TABLE(table), serverNameInstance, 1, 2, 9, 10,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);

  label = gtk_label_new (_("Negotiation timeout (sec:msec)"));
  gtk_misc_set_alignment (GTK_MISC(label), 0, 0.5);
  gtk_table_attach (GTK_TABLE(table), label, 0, 1, 10, 11, GTK_EXPAND
      | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  GtkWidget * tlsTimeOutSec;
  hbox = gtk_hbox_new (FALSE, 10);
  gtk_table_attach (GTK_TABLE(table), hbox, 1, 2, 10, 11,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  gtk_spin_button_set_value (GTK_SPIN_BUTTON(tlsTimeOutSec), g_ascii_strtod (
      negotiation_timeout_sec, NULL));
  ...
  gtk_spin_button_set_value (GTK_SPIN_BUTTON(tlsTimeOutMSec), g_ascii_strtod (
      negotiation_timeout_msec, NULL));
  gtk_box_pack_start_defaults (GTK_BOX(hbox), tlsTimeOutMSec);

  GtkWidget * verifyCertificateServer;
  verifyCertificateServer = gtk_check_button_new_with_mnemonic (
      _("Verify incoming certificates, as a server"));
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(verifyCertificateServer),
      g_strcasecmp (verify_server, "true") == 0 ? TRUE : FALSE);
  gtk_table_attach (GTK_TABLE(table), verifyCertificateServer, 0, 1, 11, 12,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);

  GtkWidget * verifyCertificateClient;
  verifyCertificateClient = gtk_check_button_new_with_mnemonic (
      _("Verify certificates from answer, as a client"));
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(verifyCertificateClient),
      g_strcasecmp (verify_client, "true") == 0 ? TRUE : FALSE);
  gtk_table_attach (GTK_TABLE(table), verifyCertificateClient, 0, 1, 12, 13,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);

  GtkWidget * requireCertificate;
  requireCertificate = gtk_check_button_new_with_mnemonic (
      _("Require certificate for incoming tls connections"));
  ...
  gtk_table_attach (GTK_TABLE(table), requireCertificate, 0, 1, 13, 14,
      GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
  ...
  if (gtk_dialog_run (GTK_DIALOG(tlsDialog)) == GTK_RESPONSE_ACCEPT)
    {
      ...
      g_hash_table_replace (properties, g_strdup (TLS_LISTENER_PORT), g_strdup (
          (gchar *) gtk_entry_get_text (GTK_ENTRY(tlsListenerPort))));
      g_hash_table_replace (properties, g_strdup (TLS_CA_LIST_FILE), g_strdup (
          gtk_file_chooser_get_filename (GTK_FILE_CHOOSER(caListFileChooser))));

      g_hash_table_replace (properties, g_strdup (TLS_CERTIFICATE_FILE),
          g_strdup (gtk_file_chooser_get_filename (
              GTK_FILE_CHOOSER(certificateFileChooser))));

      g_hash_table_replace (properties, g_strdup (TLS_PRIVATE_KEY_FILE),
          g_strdup (gtk_file_chooser_get_filename (
              GTK_FILE_CHOOSER(privateKeyFileChooser))));

      g_hash_table_replace (properties, g_strdup (TLS_PASSWORD), g_strdup (
          (gchar *) gtk_entry_get_text (GTK_ENTRY(privateKeyPasswordEntry))));

      g_hash_table_replace (properties, g_strdup (TLS_METHOD), g_strdup (
          (gchar *) gtk_combo_box_get_active_text (
              GTK_COMBO_BOX(tlsProtocolMethodCombo))));

      g_hash_table_replace (properties, g_strdup (TLS_CIPHERS), g_strdup (
          (gchar *) gtk_entry_get_text (GTK_ENTRY(cipherListEntry))));

      g_hash_table_replace (properties, g_strdup (TLS_SERVER_NAME), g_strdup (
          (gchar *) gtk_entry_get_text (GTK_ENTRY(serverNameInstance))));

      g_hash_table_replace (properties, g_strdup (TLS_VERIFY_SERVER), g_strdup (
          gtk_toggle_button_get_active (
              GTK_TOGGLE_BUTTON(verifyCertificateServer)) ? "true" : "false"));

      g_hash_table_replace (properties, g_strdup (TLS_VERIFY_CLIENT), g_strdup (
          gtk_toggle_button_get_active (
              GTK_TOGGLE_BUTTON(verifyCertificateClient)) ? "true" : "false"));

      g_hash_table_replace (properties, g_strdup (
          TLS_REQUIRE_CLIENT_CERTIFICATE),
          g_strdup (gtk_toggle_button_get_active (
              GTK_TOGGLE_BUTTON(requireCertificate)) ? "true" : "false"));

      g_hash_table_replace (properties, g_strdup (TLS_NEGOTIATION_TIMEOUT_SEC),
          g_strdup ((gchar *) gtk_entry_get_text (GTK_ENTRY(tlsTimeOutSec))));

      g_hash_table_replace (properties,
          g_strdup (TLS_NEGOTIATION_TIMEOUT_MSEC), g_strdup (
              (gchar *) gtk_entry_get_text (GTK_ENTRY(tlsTimeOutMSec))));
    }
